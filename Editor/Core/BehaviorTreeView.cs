...
namespace Kurisu.AkiBT.Editor
{
    public class BehaviorTreeView: GraphView,ITreeView
    {
        ...
        public event System.Action<SharedVariable> OnPropertyNameChange;
        /// <summary>
        ...
        private string CopyOperation(IEnumerable<GraphElement> elements)
        {
            ...
            {
                if(n is BehaviorTreeNode&&!(n as BehaviorTreeNode).Copyable)continue;
                AddToSelection(n);
            }
            return "Copy Nodes";
        }
        ...
        private void CopyFromObject(UnityEngine.Object data,Vector3 mousePosition)
        {
            ...
            if(data is TextAsset)
            {
                if(CopyFromJsonFile(((TextAsset)data).text,mousePosition))
        ...
        protected virtual void OnCommit(IBehaviorTree behaviorTree)
        {
            ...
            {
                var nodes = block.containedElements.OfType<BehaviorTreeNode>()
                .Select(x => x.GUID)
                .ToList();
